...
function txt {
    ...
    if ($erro2) {
        ...
        Write-Host "`nArquivo exportado para a pasta Documentos (%userprofile%\Documentos)." -ForegroundColor DarkCyan
...
function csv {
    ...
    if ($erro2) {
        ...
        $file | Export-CSV -Delimiter ';' -Path "$Env:USERPROFILE\documents\AD_$user.csv"
        ...
        Write-Host "`nArquivo exportado para a pasta Documentos (%userprofile%\Documentos)." -ForegroundColor DarkCyan
        Invoke-Expression "$Env:USERPROFILE\documents\AD_$user.csv"
        Start-Sleep -s 1
        return selec
    }
...
Write-Host "	[q] para fechar o script" -ForegroundColor Red              #-ForegroundColor DarkGreen
